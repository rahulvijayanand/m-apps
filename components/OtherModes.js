...
const Othermodes=({onlinepay,other})=>{
  ...
  function handleopt1(index) {
   const newI1=[...opt1];
    ...
    newI1[index]=true;
    {newI1.map((item,i)=>
     i!=index?setopt1(newI1[i]=false):null
      )}
    setopt1(newI1);
  }
    /*
 ...
 const [opt2, setopt2] = useState([false,false,false]);
  ...
  function handleopt2(index) {
   const newI2=[...opt2];
    ...
    newI2[index]=true;
    {newI2.map((item,i)=>
     i!=index?setopt2(newI2[i]=false):null
      )}
  ...
  return(
    ...
    <View>
     ...
     {onlinepay.map((item,index)=>(
       ...
       <MaterialCommunityIcons name={opt1[index]?"radiobox-marked":"radiobox-blank"} size={18} color={opt1[index]?"black":"#263D2C"} />
    </TouchableOpacity>
            ...
            > 
              ...
              <Text style={{ fontSize: 14,marginLeft:"4%",marginBottom:"2%" }}>
                {item.name}
              </Text>
     ...
     {other.map((item,index)=>(
       ...
       <MaterialCommunityIcons name={opt2[index]?"radiobox-marked":"radiobox-blank"} size={18} color={opt2[index]?"black":"#263D2C"} />
    </TouchableOpacity>
             <View
              style={{ flex:2 }}
            > 
             <Text style={{ fontSize: 14,marginLeft:"4%",marginBottom:"2%",fontWeight:"bold" }}>
                {item.name}
              </Text>
           {index==0||index==2?
            <View style={{marginLeft:"4%"}}>
             <Text>{index==0?item.info:null} 
              <Text style={{color: "#82acd9"}}>{index==0?item.l:null}</Text>
              </Text>
